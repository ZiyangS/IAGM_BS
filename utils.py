...
def get_pixel_list(img_list, eva_number, i_dict, j_dict):
    ...
    for e in range(eva_number):
        for index, img in enumerate(img_list):
            pixel_list[e][index] = img[i_dict[e]][j_dict[e]]
    return pixel_list
...
def read_ROI(ROI_path):
    results_img = cv.imread(ROI_path, 0)
    eva_number = 0
    i_dict = {}
    j_dict = {}
    for i in range(results_img.shape[0]):
        for j in range(results_img.shape[1]):
            if results_img[i][j] != 0:
                i_dict[eva_number] = i
                j_dict[eva_number] = j
                eva_number += 1
    print(eva_number)
    return eva_number, i_dict, j_dict